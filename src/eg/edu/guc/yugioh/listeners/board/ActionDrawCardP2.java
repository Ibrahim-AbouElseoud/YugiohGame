...
public class ActionDrawCardP2 implements ActionListener { //listener to draw one card
	//Note must create one for P1 & another different for P2 then remove the listener
		...
		public void actionPerformed(ActionEvent e) {
			...
			Card p2Card =Main.getController().getBoard().getActivePlayer().getField().getHand().get(index);//since the .size is already +1
			...
			Main.getGameBoard().getDeckBP2().setText(""+Main.getController().getBoard().getActivePlayer().getField().getDeck().getDeck().size());
